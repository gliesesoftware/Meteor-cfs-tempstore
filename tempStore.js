...
function mountStorage() {
  ...
  if (Package["gliese:cfs-gridfs"] && (Package["gliese:cfs-worker"] || !Package["gliese:cfs-filesystem"])) {
    ...
    // Use the gridfs
    FS.TempStore.Storage = new FS.Store.GridFS('_tempstore', { internal: true });
  } else if (Package["gliese:cfs-filesystem"]) {

    // use the Filesystem
    FS.TempStore.Storage = new FS.Store.FileSystem('_tempstore', { internal: true });
  } else {
    throw new Error('FS.TempStore.Storage is not set: Install gliese:cfs-filesystem or gliese:cfs-gridfs or set it manually');
  }
